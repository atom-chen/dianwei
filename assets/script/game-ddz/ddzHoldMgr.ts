...
@ccclass
export default class DdzHoldMgr extends cc.Component {
    @property(cc.Node)
    ...
    private cardTools: DdzCardTools;
    ...
    setHoldsSort(playerCardData?: number[]) {
        ...
        for (let idx = 0; idx < this.nodeCard.childrenCount; idx++) {
            ...
            if (idx < this._holdsCardData.length) {
                ...
                if (this.game.playerMgr.me.isDealer && idx === this._holdsCardData.length - 1) {
    ...
    removePlayCards(outCardData: number[]) {
        ...
        // 自动出牌
        if (this._holdsCardData.length === this.AUTO_DRAW_NUM) {
            this.game.showAutoBtn();
        }
    }
