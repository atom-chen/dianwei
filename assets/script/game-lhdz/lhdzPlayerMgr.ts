...
@ccclass
export default class LhdzPlayerMgr extends PlayerMgr<LhdzPlayer> {
    ...
    constructor(protected game: LhdzGame) {
        super(game);
        this._playerInfoArr = [];
    }
    ...
    updatePlayer(p: LhdzPlayer, data: PlayerInfo) {
    ...
    private getEmptySeat(): LhdzPlayer {
        let emptyPlayerArr = this.playerArr.filter((player: LhdzPlayer) => {
    ...
    private findChgPlayer() {
        ...
        for (let playerIdx = 0; playerIdx < this._playerInfoArr.length; playerIdx++) {
            ...
            if (!serverPlayer && rPos !== this.bigRegalPos && rPos !== this.gambleGodPos && rPos !== this.getMePos()) {
                // cc.log("找到桌子位置");
     ...
     * @param player 富豪或赌神
     */
    private chgBigRegalGambleGod(oldPos: number, newPos: number, player: LhdzPlayer) {
        // 先把新富豪从桌子中移除
