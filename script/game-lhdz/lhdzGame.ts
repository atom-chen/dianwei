import LhdzPlayerMgr from "./lhdzPlayerMgr";
import LhdzPlayer from "./lhdzPlayer";
import LhdzAudio from "./lhdzAudio";
import LhdzCard from "./lhdzCard";
import LhdzRecord from "./lhdzRecord";
import LhdzTrend from "./lhdzTrend";
import LhdzOther from "./lhdzOther";
import LhdzMsg, { GameStatus, LhdzUser, LhdzCardsInfo, areaBetInfo } from "./lhdzMsg";
...
@ccclass
export default class LhdzGame extends Game {
    @property([cc.Node])
    ...
    @property(cc.Prefab)
    private preAnimVs: cc.Prefab = undefined;
    ...
    @property(LhdzPlayer)
    fhPlayer: LhdzPlayer = undefined;// 富豪

    @property(LhdzPlayer)
    dsPlayer: LhdzPlayer = undefined;// 赌神


    @property(LhdzRecord)
    lhdzRecord: LhdzRecord = undefined;

    @property(LhdzTrend)
    lhdzTrend: LhdzTrend = undefined;

    @property(LhdzOther)
    lhdzOther: LhdzOther = undefined;

    @property
    private maxAreaCount: number = 50; // 区域允许的最大筹码数

    gameName = GameId.LH;
    playerMgr: LhdzPlayerMgr;
    msg: LhdzMsg;
    audioMgr: LhdzAudio;
    ...
    private MAX_AREA_BET: number = 20000;                                     // 龙虎单区域最大下注额
    ...
    private betTimePanelPosY: number;                                           // 记录闹钟初始Y值
    private nodeVsposY: number;                                                 // 记录VS初始Y值
    ...
    clearRecords() {
        this.setRecords([]);
        this.lhdzTrend.clearTrend();
        this.lhdzRecord.clearRecord();
    }
    ...
    onLoad() {
        ...
        this.cardsScale = this.lhCards[0].scale;
    ...
    initGame(): void {
        cc.log("初始化 init ");
        this.waitTips.active = false;
        this.msg = new LhdzMsg(this);
        this.msg.init();
        this.playerMgr = new LhdzPlayerMgr(this);
        this.menu.init(this);
        this.lhdzTrend.hide();
        this.lhdzOther.setGame(this);
        this.lhdzOther.hide();
     ...
     * @param isQuick
     */
    async drawCards(cardsInfo: LhdzCardsInfo[], isQuick: boolean = false) {
        cardsInfo.sort((a, b) => {
            return a.area - b.area;
        })

        for (const info of cardsInfo) {
            let nodeCard = this.lhCards[info.area];
            nodeCard.active = true;

            if (isQuick) {
                this.cardTurnAnim(nodeCard, info.card, false);
            } else {
                await this.cardTurnAnim(nodeCard, info.card, true);
                this.audioMgr.playPoint(info.card);
            }
        }
    }
    ...
    userDoBets(rPos: number, area: number, bets: string) {
        let player: LhdzPlayer = this.getBetPlayer(rPos);
        ...
        if (player) {
            ...
            if (player.isMe) {
                // 记录自己龙虎下注区域
                if (this._currBetArea === undefined && area !== Area.Peace) {
    ...
    cleanPlayerBet(rPos: number, betPointAll: areaBetInfo[]) {
        ...
        for (const betPointInfo of betPointAll) {
            ...
            // 减少总共下注金额
            totalBets = util.add(totalBets, betPoint);
     ...
     * 获取下注玩家
     */
    getBetPlayer(rPos: number): LhdzPlayer | undefined {
        if (rPos === undefined)
            return;
     ...
     * @param users
     */
    async userStatistics(winArea: number, users: LhdzUser[]) {
        // 筹码飞向胜利区域
        if (users && users.length > 0) {
            ...
            for (let index = 0; index < users.length; index++) {
                ...
                if (+userInfo.chgMoney < 0 && !player.isMe) {
                    continue;
                }
                ...
                for (const betArea of betAreaArr) {
                    if (betArea === winArea || winArea === Area.Peace) {
                        let areaPoint = 0;
                        for (const betInfo of betInfos) {
                            if (betInfo.area === betArea) {
                        ...
                        this.clipsFlyPlayer(areaPoint, betArea, pPos);
                    }
                }
        ...
        this.audioMgr.playWinArea(winArea)
        ...
        if (users) {
            for (const user of users) {
                ...
                if (betInfos) {
                    ...
                    if (+user.chgMoney !== 0 || winArea === Area.Peace) {
     ...
     * @param winArea
     */
    setWinAreaEff(winArea: number) {
        this.setAreaEff(winArea);
    }
    ...
    private onClickArea(ev: cc.Event.EventTouch, idx: string) {
        ...
        if (this.gameState === GameStatus.BET) {
            if (this._currBetPoint) {
                ...
                let maxBet = (Area.Peace === areaIdx) ? this.MAX_SPECIAL_AREA_BET : this.MAX_AREA_BET;
    ...
    */
    ...
    private getAreaRandomPos(areaIdx: number): cc.Vec2 {
        ...
        let y = cc.randomMinus1To1() * (areaSize.height - 80) * 0.43 + areaY;
        y = y - 20;
    ...
    // --------------------------动画
    async playWinAnim(player: LhdzPlayer) {
