...
import JDNNGame, { GameState, BullType } from "./jdnnGame";
import { PlayerStates } from "./jdnnPlayer";
...
export default class JDNNMsg extends GameMsg {
    loadGameHandler = "game.JDNNHandler.loadGameInfo";
    notifyCurrentGame = "jdnnNotifyCurrentGameInfo";

    protected game: JDNNGame;
    ...
    protected addExtraListeners(): void {
        this.listen("jdnnGamePhaseChange", this.handlePhaseChange);
        this.listen("jdnnDealCards", this.handleDeal);
        this.listen("jdnnDoBankBroadcast", this.handleGrabDealer);
        this.listen("jdnnAppointBanker", this.handleShowDealer);
        this.listen("jdnnDoBetBroadcast", this.handleBet);
        this.listen("jdnnBroadcastAllBetInfos", this.handleAllBet);
        this.listen("jdnnNotifyBullResult", this.handleBullResult);
        this.listen("jdnnDoHandoutBroadcast", this.handleCalculateFinish);
        this.listen("jdnnDoResultBroadcast", this.handleResult);
    }
    ...
    private handlePhaseChange(data: { state: number; timer: number }, isReconnect = false) {
        ...
        //补5张
        if (state >= GameState.Deal && state < GameState.Balance) {
            if (me.isLooker && this._showForLooker) {
                this._showForLooker = false;
                let args = { cardRunes: [0, 0, 0, 0, 0] };
    ...
    sendBet(val: number) {
        window.pomelo.notify("game.JDNNHandler.doBet", { betPoint: val });
    }

    sendGrab(val: number) {
        cc.log("send grab")
        window.pomelo.notify("game.JDNNHandler.doBank", { bankPoint: val });
    }

    //计算完成
    sendFinish() {
        window.pomelo.notify("game.JDNNHandler.doResult", {});
    }
}